...
//             string fileName=@"d:\abhi\products.json";
...
//     finally{ }
    ...
    try{
            ...
            // var productsJson = JsonSerializer.Serialize<List<Product>>(flowers,option);

            String filename=@"d:\abhi\abhiproduct.json";

            File.WriteAllText(filename,JsonSerializer.Serialize<List<Product>>(flowers,new JsonSerializerOptions{IncludeFields=true}));
            //Deserialization......
            String Jsons = File.ReadAllText(filename);

            List<Product> Jsonflowers = JsonSerializer.Deserialize<List<Product>>(Jsons);
            foreach(Product p in Jsonflowers){
...
// File.WriteAllText(filename,JsonSerializer.Serialize<List<Prodcut>>(flowers,new JsonSerializerOptions{IncludeFields=true}));